...
def Execute(data):
    if data.IsChatMessage() and data.GetParam(0).lower():
        ...
        if startCheck and trig and Parent.HasPermission(data.User, MySettings.Permission, MySettings.PermissionInfo):
            ...
            swl = literal_eval(MySettings.SecretWord)    
            while wordnum < wordcount:
                #check if word at wordnum matches secret word
                if data.GetParam(wordnum) in swl:
                    #give user points
